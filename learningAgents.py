...
class ReinforcementAgent(ValueEstimationAgent):
    ...
    def __init__(self, actionFn=None, numTraining=100, epsilon=0.25, alpha=0.25, gamma=0.995):
        """
...
class QLearningAgent(ReinforcementAgent):
    ...
    def getValue(self, state):
        ...
        if len(legalActions) == 0:
            return 0.0
        value = self.getQValue(state, legalActions[0])
    ...
    def getAction(self, state):
        ...
        if len(legalActions) == 0:
            ...
            if util.flipCoin(self.epsilon) == 1:
    ...
    def update(self, state, action, nextState, reward):
        ...
        if len(legalActions) == 0:
            ...
            nextBestAction = legalActions[0]
            for nextAction in legalActions:
                if self.getQValue(nextState, nextAction) > self.getQValue(nextState, nextBestAction):
                    nextBestAction = nextAction
            self.Q_values[(state, action)] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * (
                    reward + self.discount * self.getQValue(nextState, nextBestAction))
...
class ApproximateQAgent(QLearningAgent):
    ...
    def __init__(self, extractor='featureExtractors.SimpleExtractor', **args):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feat in feats:
            self.weights[feat] += self.alpha * correction * feats[feat]
